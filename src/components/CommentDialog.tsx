...
import { MessageSquareCode, StarIcon } from 'lucide-react';
...
const CommentDialog = ({interviewId}:{interviewId:Id<'interviews'>}) => {
  ...
  const handleSubmit = async() =>{
    ...
    try {
      await addComment({interviewId,content:comment,rating:parseInt(rating)});
  ...
  const renderStars = (rating:number) => {
    return <div className='flex gap-0.5'>
      {[1,2,3,4,5].map((star) => (
        <StarIcon key={star} className={`h-4 w-4 ${star <= rating ? 'fill-primary text-primary' : 'text-muted-foreground'}`} />
      ))}
    </div>
  }

  if(existingComments == undefined || users === undefined) return <LoaderUI/>
